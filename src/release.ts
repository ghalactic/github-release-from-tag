  ...
  return group(
    ...
    > => {
      ...
      if (!tag.match(SEMVER_PATTERN)) {
        info(`The tag "${tag}" is not valid SemVer`);
        info("This release will not be set as the latest");

        return ["false", "false"];
      }

      info(`The tag "${tag}" is valid SemVer`);
      ...
      if (!currentTag.match(SEMVER_PATTERN)) {
        info(
          `The current latest release tag "${currentTag}" is not valid SemVer`,
        );
        info("This release will be set as the latest");
      ...
      if (comparison === 0) {
        info(
          `The tag "${tag}" has equal SemVer precedence to ` +
            `the current latest release tag "${currentTag}"`,
        );
        info(
          "This release will be set as the latest only if it's newly created",
        );
      ...
      if (comparison > 0) {
        info(
          `The tag "${tag}" has higher SemVer precedence than ` +
            `the current latest release tag "${latestRelease.tag_name}"`,
        );
        info("This release will be set as the latest");
      ...
      info(
        `The tag "${tag}" has lower SemVer precedence than ` +
          `the current latest release tag "${latestRelease.tag_name}"`,
      );
      info("This release will not be set as the latest");
...
export async function compareLatestRelease({
  ...
  const [latestRelease, isLatest] = await group(
    ...
    async () => {
      // TODO: Fetch current latest release, compare IDs
      const latestRelease = await getLatestRelease(repos, owner, repo);
