...
export async function listDiscussionCategories () {
  ...
  const query = `query listDiscussionCategories($owner: String!, $repo: String!, $before: String) {
    repository(owner: $owner, name: $repo) {
      discussionCategories(last: 10, before: $before) {
        edges {
          node {
            id
            name
  ...
  do {
    ...
    results.push(...edges.map(({node}) => node))
  } while (hasPreviousPage)

  return results
}

export async function* listDiscussionsByCategory (categoryName) {
  const octokit = createOctokit()

  const categories = await listDiscussionCategories()
  const category = categories.find(({name}) => name.toLowerCase() === categoryName.toLowerCase())

  if (category == null) throw new Error(`No discussion category found with name ${JSON.stringify(categoryName)}`)

  let hasPreviousPage = true
  let before = null

  const query = `query listDiscussionCategories($owner: String!, $repo: String!, $category: ID!, $before: String) {
    repository(owner: $owner, name: $repo) {
      discussions(last: 10, before: $before, categoryId: $category) {
        edges {
          node {
            title
            bodyHTML
  ...
  do {
    ...
    for (const {node} of edges) yield node
  } while (hasPreviousPage)
}
