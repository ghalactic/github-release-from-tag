...
semVerFixtures.valid = [...semVerFixtures.validStable, ...semVerFixtures.validUnstable]

describe('parseRef()', () => {
  it('should be able to detect tag refs', () => {
    expect(parseRef('refs/tags/a')).toMatchObject({isTag: true, tag: 'a'})
    expect(parseRef('refs/tags/1.1.1')).toMatchObject({isTag: true, tag: '1.1.1'})
  })
  ...
  it.each(semVerFixtures.valid)('should be able to detect SemVer tag refs with a "v" prefix (v%s)', tag => {
    expect(parseRef(`refs/tags/v${tag}`)).toMatchObject({isSemVer: true})
  })

  it.each(semVerFixtures.invalid)('should be able to detect non-SemVer tag refs (%s)', tag => {
    expect(parseRef(`refs/tags/${tag}`)).toMatchObject({isSemVer: false})
  })

  it.each(semVerFixtures.invalid)('should be able to detect non-SemVer tag refs with a "v" prefix (v%s)', tag => {
    expect(parseRef(`refs/tags/v${tag}`)).toMatchObject({isSemVer: false})
  })

  it.each(semVerFixtures.validStable)('should be able to detect stable SemVer tag refs (%s)', tag => {
    expect(parseRef(`refs/tags/${tag}`)).toMatchObject({isStable: true})
  })

  it.each(semVerFixtures.validStable)('should be able to detect stable SemVer tag refs with a "v" prefix (v%s)', tag => {
    expect(parseRef(`refs/tags/v${tag}`)).toMatchObject({isStable: true})
  })

  it.each(semVerFixtures.validUnstable)('should be able to detect unstable SemVer tag refs (%s)', tag => {
    expect(parseRef(`refs/tags/${tag}`)).toMatchObject({isStable: false})
  })
})
