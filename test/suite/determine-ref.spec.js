...
describe('determineRef()', () => {
  ...
  describe('Happy path', () => {
    beforeEach(async () => {
      ...
      await execGit('-C', paths.origin, 'commit', '--quiet', '--allow-empty', '--message=commit-message-b')
      await execGit('-C', paths.origin, 'tag', '--annotate', '--message=tag-message-a', 'tag-a')
      await execGit('-C', paths.origin, 'commit', '--quiet', '--allow-empty', '--message=commit-message-c')
      ...
      await execGit('-C', paths.clone, 'fetch', 'origin', 'refs/tags/tag-a:refs/tags/tag-a')
      await execGit('-C', paths.clone, 'fetch', 'origin', 'refs/heads/branch-a:refs/heads/branch-a')
    ...
    it('should determine the ref when an annotatd tag is checked out', async () => {
      ...
      expect(await determineRef({group, info, silent})).toEqual('refs/tags/tag-a')
    })

    it('should determine the ref when a lightweight tag is checked out', async () => {
      await execGit('-C', paths.clone, 'switch', '--quiet', '--detach', 'tag-b')

      expect(await determineRef({group, info, silent})).toEqual('refs/tags/tag-b')
    })

    it('should determine the ref when a branch is checked out', async () => {
      await execGit('-C', paths.clone, 'switch', '--quiet', 'branch-a')

      expect(await determineRef({group, info, silent})).toEqual('refs/heads/branch-a')
    })
  })
  ...
  describe('Unhappy path', () => {
    ...
    it('should fail to determine the ref', async () => {
      await expect(() => determineRef({group, info, silent})).rejects.toThrow()
    })
  })
})
