...
import {createAlreadyExistsError, createError, createRepos} from '../mocks/oktokit-repos.js'
...
describe('createOrUpdateRelease()', () => {
  ...
  it('should create a new release when no matching release exists', async () => {
    ...
    const [actual, wasCreated] = await createOrUpdateRelease({...staticParams, repos})
  ...
  it('should update the existing release when a matching release exists', async () => {
    ...
    expect(actual).toMatchObject({...defaultExpectation, release_id: releaseId})
  })

  it('should honor the isStable option', async () => {
    const repos = createRepos()

    const [actual, wasCreated] = await createOrUpdateRelease({...staticParams, repos, isStable: false})

    expect(wasCreated).toBe(true)
    expect(actual).toMatchObject({...defaultExpectation, prerelease: true})
  })

  it('should honor the isDraft option', async () => {
    const repos = createRepos()

    const [actual, wasCreated] = await createOrUpdateRelease({...staticParams, repos, isDraft: true})
  ...
  describe('error propagation', () => {
    it('should propagate errors during release creation', async () => {
      ...
      await expect(() => createOrUpdateRelease({...staticParams, repos})).rejects.toThrow(error)
    })
